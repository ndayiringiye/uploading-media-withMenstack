...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post("http://localhost:4000/user/login", dataForm);
      console.log(response.data);
      alert("Login successful! Please go home page.");
      navigate("/home");
    } catch (error) {
  ...
  return (
    <div className="flex justify-center my-20 bg-slate-500 w-1/2 mx-auto py-6">
      <div className="flex flex-col gap-y-4">
        <div>
          <p className="text-xl font-semibold">just login an account on my website</p>
        </div>
